...
var cheater_run = setInterval(function() {
    if (checkExist('.modal-body h2')) {
        if (findByAttribute('h2', 'ng-bind-html', 'question')) {
            var question = find('.modal-body h2').textContent;
            var answers = findAll('.modal-body li');
            console.log("Question:", question);
            if (!(question in training)) {
                training[question] = 0;
                console.log("Answer:", answers[0].textContent);
                find('input', answers[0]).click();
            } else if (test_attempts > 0) {
            console.log("Answer:", answers[training[question]].textContent);
                find('input', answers[training[question]]).click();
            }
            if (findByText('div.qnav button', 'Next')) {
                if (!findByText('div.qnav button', 'Next').disabled) {
                    findByText('div.qnav button', 'Next').click();
                } else if (!findByText('div.comp button', 'Done').disabled) {
                    findByText('div.comp button', 'Done').click();
                }
            }
        }
        // Runs if finished quiz
        if (checkExist('div.no-m-bottom')) {
            var wrong_questions = findAll('div.no-m-bottom li');
            ...
            test_attempts++;
            findByText('div.comp button', 'Close').click();
            sleep(250).then(() => {
                if (findByText('button.btn-confirm', 'Take Quiz')) {
                    findByText('button.btn-confirm', 'Take Quiz').click();
                } else if (findByText('button.completed', 'Completed Article')) {
                    clearInterval(cheater_run);
                }
            });
        }
    }
...
function checkExistInFrame(doc, css, element=false) {
    if (findAllInFrame(css, element).length > 0) {
        return true;
    } else {
        return false;
    }
